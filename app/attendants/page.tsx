    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      <div className="anim-fade-up" style={{ flex: 1, overflowY: "auto", padding: "clamp(16px, 3vw, 28px)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "20px", flexWrap: "wrap" }}>
          ...
          {!loading && <span style={{ fontSize: "13px", color: "var(--text-dim)" }}>{filtered.length} attendant{filtered.length !== 1 ? "s" : ""}</span>}
        </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px", minWidth: "480px" }}>
              ...
              <tbody>
                {loading ? <TableSkeleton /> : paginated.length === 0 ? (
                  <tr><td colSpan={isAdmin ? 5 : 4} style={{ padding: "60px 12px", textAlign: "center", color: "var(--text-dim)" }}>No attendants found.</td></tr>
                ) : paginated.map((a: any) => {
                  const shiftStyle = shiftColors[a.shift] || shiftColors["Morning"];
                  return (
                    <tr key={a._id} className="table-row">
                      <td style={{ padding: "13px 14px", color: "var(--text)", fontWeight: 500, borderBottom: "1px solid var(--hover-bg)" }}>{a.name}</td>
                      <td style={{ padding: "13px 14px", color: "var(--text-muted)", borderBottom: "1px solid var(--hover-bg)", fontFamily: "monospace", fontSize: "12px" }}>{a.staffId || "—"}</td>
                      ...
                      <td style={{ padding: "13px 14px", color: "var(--text-muted)", borderBottom: "1px solid var(--hover-bg)" }}>{a.email || "—"}</td>
                      {isAdmin && (
                        <td style={{ padding: "13px 14px", borderBottom: "1px solid var(--hover-bg)" }}>
          ...
          {totalPages > 1 && (
            <div style={{ display: "flex", gap: "8px", padding: "14px 16px", justifyContent: "flex-end", alignItems: "center", borderTop: "1px solid var(--hover-bg)" }}>
              <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="btn-secondary"
                style={{ background: "var(--surface2)", border: "1px solid var(--border)", color: page === 1 ? "var(--text-disabled)" : "var(--text-muted)", padding: "6px 14px", borderRadius: "7px", fontFamily: "inherit", fontSize: "13px" }}>← Prev</button>
              ...
              <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="btn-secondary"
                style={{ background: "var(--surface2)", border: "1px solid var(--border)", color: page === totalPages ? "var(--text-disabled)" : "var(--text-muted)", padding: "6px 14px", borderRadius: "7px", fontFamily: "inherit", fontSize: "13px" }}>Next →</button>
            </div>
          )}
        </div>
      </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.72)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, backdropFilter: "blur(4px)" }}
          ...
          <div className="anim-scale-in modal-inner" style={{ background: "var(--surface)", border: "1px solid var(--border-hover)", borderRadius: "18px", width: "460px" }}
            ...
            <div style={{ padding: "22px 26px 16px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <h2 style={{ fontSize: "19px", color: "var(--text)" }}>Add Attendant</h2>
            ...
            <div style={{ padding: "20px 26px" }}>
              ...
              <div style={{ marginBottom: "16px" }}>
                <label style={{ display: "block", fontSize: "11px", color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: "1px", marginBottom: "7px", fontWeight: 600 }}>Shift</label>
